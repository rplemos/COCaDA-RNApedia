...
def process_file(file_path):
    ...
    try:
        ...
        contacts_list, luana_res, luana_atom, puton_res = contacts.contact_detection(parsed_data)
        process_time = timer() - start_time
        return parsed_data, contacts_list, process_time, luana_res, luana_atom, puton_res
...
def process_result(result, output):
    ...
    """
    if result:
        protein, contacts_list, process_time, luana_res, luana_atom, puton_res = result
        output_data = f"ID: {protein.id} | Protein Size: {protein.true_count():<7} | Luana Atom: {luana_atom:<7}  | Luana Res = {luana_res:<7} | Puton = {puton_res:<7} | Time: {process_time:.3f}s"
        print(output_data)
